...
foreach ($caratteri as $lettalf) {
 ...
 if ($lettalf==' ') {
  print ('<IMG SRC="casti/stah/data/hgman/images/lb_~.gif" BORDER=0 WIDTH=20 HEIGHT=20 ALT="">');
 }
 else {
  print ('<IMG SRC="<IMG SRC="casti/stah/data/hgman/images/lb_'.$lettalf.'.gif" BORDER=0 WIDTH=20 HEIGHT=20 ALT="'.$lettalf.'">');
 }
 print ('<IMG SRC="<IMG SRC="casti/stah/data/hgman/images/lb_~.gif" BORDER=0 WIDTH=20 HEIGHT=20 ALT="">');
}
print ('<BR><BR>');
...
if ($indovina!=$indovinata) {
 if ($tentativi>=6) {
  ...
  foreach ($alfabeto as $lettalf) {
   ...
   if ($contrl) {
    print ('<IMG SRC="<IMG SRC="casti/stah/data/hgman/images/lr_'.$lettalf.'.gif" BORDER=0 WIDTH=20 HEIGHT=20 ALT="'.$lettalf.'">');
   }
   else {
    print ('<A HREF="'.$HTTP_SERVER_VARS["PHP_SELF"].'?letter='.$lettalf.'&'.SID.'"><IMG SRC="images/lb_'.$lettalf.'.gif" BORDER=0 WIDTH=20 HEIGHT=20 ALT="'.$lettalf.'"></A>');
   }
   if ($lettalf=='m') {
    print ('<BR>');
   }
...
?>
<BR>
<P><?php print ('<A HREF="'.$HTTP_SERVER_VARS["PHP_SELF"].'>"'); ?>New Game</A></P>